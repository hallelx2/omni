 ...
 *
 *   - "build" (default): full tool access; the critic reviews each turn.
 ...
 * `request_build_mode`, or config) is visible everywhere.
 */
export type RunMode = "plan" | "build"
export type ModeSource = "default" | "manual" | "auto" | "agent"

/**
 * Tools the model may call while in plan mode. Read-only investigation plus the
 * escape hatch to request build mode. The engine enforces this via `enabledTools`.
 */
